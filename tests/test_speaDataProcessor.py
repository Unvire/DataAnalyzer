...
def test__processFileLine(fileLinesProcessingTest):
    ...
    serialNumber = loader._getSerialNumber(fileLines)
    fileLines = fileLines[:-3] # no header so only footer is trimmed

    for line in fileLines:
        loader._processFileLine(line, testTime, serialNumber)
    ...
    assert dataInstance.getData('1') == [
            [DataPoint(float('1.499953e+05'), (float('1.454955e+05'), float('1.544952e+05')), '2025/01/25 17:35:59', '170250254017870')]
        ]

    dataInstance = measurements['R262 | RESR262 10K 5%']
    assert list(dataInstance.data.keys()) == ['1']
    assert dataInstance.getData('1') == [
            [DataPoint(float('9.968847e+03'), (float('9.300000e+03'), float('1.070000e+04')),'2025/01/25 17:35:59', '170250254017870')]
        ]

    dataInstance = measurements['R265 | RESR265 1K 1%']
    assert list(dataInstance.data.keys()) == ['1', '2']
    assert dataInstance.getData('1') == [
            [DataPoint(float('9.960000e+02'), (float('9.300000e+02'), float('1.070000e+03')), '2025/01/25 17:35:59', '170250254017870'), 
             DataPoint(float('9.970000e+02'), (float('9.300000e+02'), float('1.070000e+03')), '2025/01/25 17:35:59', '170250254017870')]
        ]
    assert dataInstance.getData('2') == [
            [DataPoint(float('9.980000e+02'), (float('9.300000e+02'), float('1.070000e+03')), '2025/01/25 17:35:59', '170250254017870'), 
             DataPoint(float('9.990000e+02'), (float('9.300000e+02'), float('1.070000e+03')), '2025/01/25 17:35:59', '170250254017870')]
        ]
    
    dataInstance = measurements['LED4 | LED_WHITE_15704401_SW57P19D22K72_CXCY']
    assert list(dataInstance.data.keys()) == ['2']
    assert dataInstance.getData('2') == [
            [DataPoint((float('0.329'), float('0.34')), '0.3196_0.3462_0.3396_0.3616_0.3386_0.3369_0.3196_0.3198', '2025/01/25 17:35:59', '170250254017870')]
        ]